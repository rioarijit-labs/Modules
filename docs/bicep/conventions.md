...
These rules keep the modules predictable for people and for agents. `Azure/Bicep/scripts/validate.py` enforces the mechanical ones.

## Structure

- One module per folder: `res/<provider>/<module>/main.bicep`, named after the resource, not the AVM module. The folder path is the module ID (`azure:bicep:res/storage/storage-account`).
...
- Releases are tagged for the whole Bicep folder: `azure-bicep/v<major>.<minor>.<patch>`. The `azure-` prefix distinguishes it from a future `aws-cloudformation/v...` tag namespace.
