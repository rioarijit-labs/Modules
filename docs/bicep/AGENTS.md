...
1. **Use the modules in `Azure/Bicep/res/` and `Azure/Bicep/ptn/` before writing raw resources.** Only fall back to a raw resource or an AVM module directly when nothing here fits, and say so.
...
10. **Validate before you finish:** `python3 Azure/Bicep/scripts/validate.py` must pass with no warnings.
